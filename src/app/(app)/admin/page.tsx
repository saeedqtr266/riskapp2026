...
import { roleLabels } from "@/lib/risk/calculations"
...
export default async function AdminPage() {
  const { departments, categories, users, audit } = await getAdminData()
        ...
        <CardContent className="flex flex-col gap-2 text-sm">
          {audit.map((event) => (
            <div key={event.id} className="rounded-lg border border-slate-200 p-3">
              {event.actor_name ?? "System"} moved a risk from {event.from_status ?? "new"} to {event.to_status}
